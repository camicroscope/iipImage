...
void OpenSlideImage::loadImageInfo(int x, int y){
  ...
  lastTileXDim.push_back(w % tile_widths[0]);
  lastTileYDim.push_back(h % tile_heights[0]);
  numTilesX.push_back((w + tile_widths[0] - 1) / tile_widths[0]);
  numTilesY.push_back((h + tile_heights[0] - 1) / tile_heights[0]);
 ...
 */
RawTile OpenSlideImage::getTile(int seq, int ang, unsigned int iipres, int layers, unsigned int tile){
 ...
 */
RawTile OpenSlideImage::getCachedTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
 ...
 * @param res  	iipsrv's resolution id.  openslide's level is inverted from this.
 */
RawTile OpenSlideImage::getNativeTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
  ...
  size_t tw = tile_widths[0];
  size_t th = tile_heights[0];
  ...
  RawTile rt(tiley * ntlx + tilex, iipres, 0, 0, tw, th, channels, bpc);
  ...
  size_t tx0 = (tilex * tile_widths[0]) << osi_level;  // same as multiply by z power of 2
  size_t ty0 = (tiley * tile_heights[0]) << osi_level;
 ...
 */
RawTile OpenSlideImage::halfsampleAndComposeTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
  ...
  RawTile tt;
  // temp storage.
  uint8_t *tt_data = new uint8_t[(tile_widths[0] >> 1) * (tile_heights[0] >> 1) * channels];
  ...
  logfile << "OpenSlide :: halfsampleAndComoseTile() :: cache insert res " << tt_iipres << " " << ttx << "x" << tty << " :: " << timer.getTime() << " microseconds" << endl << flush;
        ...
        compose(tt_data, tt_out_w, tt_out_h, (tile_widths[0] / 2) * i, (tile_heights[0] / 2) * j,
