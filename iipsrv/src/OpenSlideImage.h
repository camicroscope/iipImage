...
//    void downsample_region(uint32_t *buf, const uint64_t x, const uint64_t y, const int32_t z, const uint32_t w, const uint32_t h);
    ...
    RawTile getCachedTile(const size_t tilex, const size_t tiley, const uint32_t iipres);

    /// read from file, color convert, store in cache, and return tile.
    RawTile getNativeTile(const size_t tilex, const size_t tiley, const uint32_t iipres);
    ...
    RawTile halfsampleAndComposeTile(const size_t tilex, const size_t tiley, const uint32_t iipres);
    ...
    OpenSlideImage() : IIPImage() {
        ...
        tile_heights.push_back(OPENSLIDE_TILESIZE);
        osr = NULL;
    };
...
public:
    ...
    /** \param x horizontal sequence angle
        ...
        \param t tile number
     */
	virtual RawTile getTile(int x, int y, unsigned int r, int l, unsigned int t);
